...
class DRGAT(nn.Module):
    ...
    def forward(self, graph, feat, return_hidden=False, node_weight=None):
        ...
        _hidden = []
        hidden_list = [h]
        h = self.convs[0](graph, h, self.perms[0], node_weight=node_weight).flatten(1, -1)
        _hidden.append(h)
        ...
        for i in range(1, self.n_layers - 1):
            ...
            h = self.convs[i](graph, h, self.perms[i], node_weight=node_weight).flatten(1, -1)
        ...
        h = self.dp_last(h)
        hidden_list.append(h)
        h = self.convs[-1](graph, h, self.perms[-1], node_weight=node_weight)   # shape: [169343, 1, 40]
        h = h.mean(1)                                       # shape: [169343, 40]
        hidden_list.append(h)
        h = self.bias_last(h)
