...
class GCN_multirelation(nn.Module):
    ...
    def forward(self, x, adjs, return_hidden=False, node_weight=None):
        ...
        if return_hidden:
            return (x2, hidden_list) if self.skip_mode == "none" else (self.skip_connect_out(x2, x1), hidden_list)
        else:
...
class TIMME(nn.Module):
    ...
    def forward(self, x, adjs, only_classify=False, return_hidden=False, node_weight=None):
        ...
        if only_classify:
            ...
            if return_hidden:
                return [m(gcn_embedding, adjs, calc_gcn=False) for m in self.models], hidden_list
            else:
